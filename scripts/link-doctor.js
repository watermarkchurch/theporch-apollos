...
function getClientPackages() {
  const packageJson = JSON.parse(
    fs.readFileSync(`${__dirname}/../ThePorch/package.json`)
  );
  return [
...
function checkWml() {
  const out = spawnSync('yarn', ['wml', 'list'], {
    encoding: 'utf8',
    cwd: `${__dirname}/../ThePorch`,
  });
  const { stdout } = out;
  if (stdout.includes('No links set')) {
    return 'No links in ThePorch have been created. Have you ran `yarn link-packages` ?';
  }
...
function checkEnvVariable() {
  try {
    ...
    const apollosAppsLocationFromEnv = fs
      .readFileSync(`${__dirname}/../ThePorch/.env`, 'utf8')
    ...
    console.log(path.resolve(__dirname, '../ThePorch/', apollosAppsLocation));
    if (
      !fs.existsSync(
        `${path.resolve(
          __dirname,
